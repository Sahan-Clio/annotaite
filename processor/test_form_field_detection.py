...
def create_debug_images(original_image, cleaned_image, text_elements, input_boxes, checkboxes, page_number, output_dir):
    ...
    input_debug_original = original_image.copy()
    
    # 3. Detected input fields on cleaned image with method labels
    input_debug_cleaned = cleaned_image.copy()
    ...
    for i, box in enumerate(input_boxes):
        ...
        cv2.rectangle(input_debug_original, (box['x'], box['y']), 
                     (box['x'] + box['width'], box['y'] + box['height']), color, 2)
        
        # Draw on cleaned image
        cv2.rectangle(input_debug_cleaned, (box['x'], box['y']), 
                     (box['x'] + box['width'], box['y'] + box['height']), color, 2)
        
        # Add method label
        label = f"{method[:4]}{i+1}"
        cv2.putText(input_debug_original, label, (box['x'], box['y'] - 5), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
        cv2.putText(input_debug_cleaned, label, (box['x'], box['y'] - 5), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
    ...
    cv2.imwrite(str(output_dir / f"page_{page_number:03d}_04_detected_on_original.png"), input_debug_original)
    cv2.imwrite(str(output_dir / f"page_{page_number:03d}_05_detected_on_cleaned.png"), input_debug_cleaned)
    
    print(f"  🎨 Saved debug images for page {page_number} (including detection on cleaned image)")
...
def process_pdf_for_form_fields(pdf_path: Path, output_dir: Path = None):
    """Process PDF to detect form fields using combined layout + CV approach."""
    try:
        from pdf2image import convert_from_path
        
        # Set up output directory
        if output_dir is None:
            output_dir = pdf_path.parent / "form_field_detection"
        output_dir.mkdir(exist_ok=True)
        
        print("🎯 Form Field Detection (Layout + Computer Vision)")
        print("=" * 60)
